...
export default function Jadwal() {
  ...
  const loadPasien = async () => {
    try { const r = await pasienAPI.fetchAll(); setPasienList(r) }
    catch (err) { console.error('Gagal load pasien:', err) }
  }

  const handleChange = e => {
    const { name, value } = e.target
    if (name === 'pasien_id') {
      const pasien = pasienList.find(p => String(p.id) === value)
      setForm({ ...form, pasien_id: value, nama_pasien: pasien?.nama_lengkap || '' })
    } else {
  ...
  const handleOpenEdit = j => {
    setEditId(j.id)
    setForm({ pasien_id: String(j.pasien_id || ''), nama_pasien: j.nama_pasien, dokter: j.dokter, tanggal: j.tanggal, jam: j.jam, jenis_perawatan: j.jenis_perawatan, status: j.status, catatan: j.catatan || '' })
    setShowModal(true)
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[['Terjadwal','text-biru'],['Selesai','text-hijau'],['Dibatalkan','text-merah']].map(([s, cls]) => (
          <div key={s} className="bg-white rounded-2xl shadow-[0_2px_15px_rgba(0,0,0,0.06)] p-5">
            ...
            <p className={`text-3xl font-bold ${cls}`}>{countByStatus(s)}</p>
          </div>
        ))}
      </div>
      ...
      <Modal isOpen={showModal} onClose={() => { setShowModal(false); setEditId(null) }}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-teks-samping uppercase tracking-wider mb-1.5">Pilih Pasien</label>
            <select name="pasien_id" value={form.pasien_id} onChange={handleChange}
              className="w-full px-3 py-2.5 border border-garis rounded-xl text-sm outline-none focus:border-biru bg-white text-teks">
